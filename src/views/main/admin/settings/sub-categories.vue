<template>
  <Layout>
    <BRow>
      <BCol lg="12">
        <BCard no-body id="subCategoryList">
          <!-- Header -->
          <BCardHeader class="border-0">
            <BRow class="align-items-center gy-3">
              <BCol cols="sm">
                <h5 class="card-title mb-0">Settings | Sub Categories</h5>
              </BCol>
              <BCol sm="auto">
          ...
          >
            ...
            >
              <BLink
                class="nav-link py-3"
                :class="{ active: selectedType === category.slug }"
                data-bs-toggle="tab"
                role="tab"
                @click="changecategory(category)"
              >
                <i :class="[category.icon, 'me-1', 'align-bottom']"></i>
                {{ category.name }}
...
import {
  getSettingSubCategories,
  createSettingSubCategory,
  updateSettingSubCategory,
  deleteSettingSubCategory,
} from "@/api-request";
...
const fetchDataTable = async (pageNumber = 1) => {
  ...
  try {
    const response = await getSettingSubCategories({
    ...
    if (!selectedType.value && categories.value.length) {
      selectedType.value = categories.value[0].slug;
    }
  } catch (error) {
    console.error(error);
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = editId.value
      ? await updateSettingSubCategory(editId.value, payload)
      : await createSettingSubCategory(payload);
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await deleteSettingSubCategory(deleteItem.value.id);
