<template>
  <Layout>
    <BRow>
      <BCol lg="12">
        <BCard no-body id="categoryList">
          <!-- Header -->
          <BCardHeader class="border-0">
            <BRow class="align-items-center gy-3">
              <BCol cols="sm">
                <h5 class="card-title mb-0">Settings | Main Categories</h5>
              </BCol>
              <BCol sm="auto">
...
const fetchDataTable = async (pageNumber = 1) => {
  ...
  try {
    const response = await getSettingCategories({
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = editId.value
      ? await updateSettingCategory(editId.value, form.value)
      : await createSettingCategory(form.value);
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await deleteSettingCategory(deleteItem.value.id);
