  ...
  <Layout>
    <BRow class="gy-4">
                ...
                >
                  ...
                  >
                    <i
                      ...
                      "
                    ></i>
                    {{ job.is_applied ? "Already Applied" : "View Details" }}
                  </BButton>
                </div>
                    ...
                    >
                      <div class="avatar-xs flex-shrink-0">
                        <span class="avatar-title bg-white rounded-circle">
                          <i :class="detail.icon + ' text-primary fs-18'"></i>
                        </span>
                      </div>
                      <div class="flex-grow-1 ms-2">
                        <span class="fs-12 text-muted">{{ detail.label }}</span>
                        <div
      ...
      <BCol lg="3">
        <BCard
          no-body
          class="job-list-view-card overflow-hidden sticky-side-div"
          id="job-overview"
        >
          <img
            :src="
            ...
            <div class="avatar-md mt-n5">
              <div class="avatar-title bg-light rounded-circle">
                <img
                  ...
                  "
                  alt=""
                  class="rounded-circle avatar-md view-companylogo"
                />
              </div>
            </div>
            <div class="mt-3">
              <h4 class="view-title mb-2">{{ currentUser?.name }}</h4>

              <ul class="list-unstyled text-muted mb-3">
                <li
                  v-if="currentUser?.job_seeker?.field_of_study"
                  class="d-flex align-items-center gap-2 mb-2"
                >
                  <i class="ri-book-2-line text-primary"></i>
                  <span>{{ currentUser.job_seeker.field_of_study }}</span>
                </li>

                <li
                  v-if="currentUser?.job_seeker?.education_level"
                  class="d-flex align-items-center gap-2 mb-2"
                >
                  <i class="ri-graduation-cap-line text-info"></i>
                  <span>{{ currentUser.job_seeker.education_level }}</span>
                </li>

                <li
                  v-if="currentUser?.job_seeker?.preferred_location"
                  class="d-flex align-items-center gap-2 mb-2"
                >
                  <i class="ri-map-pin-2-line text-warning"></i>
                  <span>{{ currentUser.job_seeker.preferred_location }}</span>
                </li>

                <li
                  v-if="currentUser?.job_seeker?.years_of_experience"
                  class="d-flex align-items-center gap-2 mb-2"
                >
                  <i class="ri-briefcase-line text-success"></i>
                  <span
                    >{{ currentUser.job_seeker.years_of_experience }} years
                    experience</span
                  >
                </li>

                <li
                  v-if="currentUser?.job_seeker?.is_available !== undefined"
              ...
              <!-- Bio -->
              <p class="text-muted mb-0" v-html="truncatedBio"></p>
            </div>
          </BCardBody>
          ...
          <BCardBody class="border-top">
            ...
            <div class="d-flex flex-wrap gap-2 fs-15">
              <template v-if="currentUser?.job_seeker?.skills">
                <BLink
                  v-for="(skill, index) in currentUser.job_seeker.skills"
                  :key="index"
                  href="javascript:void(0);"
                  class="badge bg-primary-subtle text-primary"
                >
...
const truncatedBio = computed(() => {
  if (!currentUser?.value?.bio) return "";

  let bio = currentUser.value.bio;

  // Limit to 300 characters
  if (bio.length > 350) {
    bio = bio.substring(0, 350) + "...";
  }
...
//   console.log("Toggle save job:", job.id);
// };
...
const showdetail = (job) => {
  router.push(`/jobs-details/${job.code}`);
};
...
const fetchJob = async (append = false) => {
  ...
  try {
    const response = await getSeekerJobLists({
      page: page.value,
      per_page: 3,
    ...
    if (append) {
      jobDatas.value.push(...response.data);
    } else {
      jobDatas.value = response.data;
    }

    total.value = response.total;
    displayed.value = response.displayed;

    hasMore.value =
      response.current_page < Math.ceil(response.total / response.per_page);
  } catch (error) {
    console.error(error);
    Toast.fire({
      icon: "error",
...
onMounted(async () => {
  ...
  // Watch when loading completes (sentinel will appear then)
  watch(
    () => isLoading.value,
    (loading) => {
